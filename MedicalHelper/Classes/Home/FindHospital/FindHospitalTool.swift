...
class FindHospitalTool: NSObject {
    ...
    class func  findHospitalListData(page: Int,finished: (responseObject: [FindHospitalModel]?, error: NSError?) -> ()){
        ...
        let kSignStr1 = "pbNypxbuiT"
        var sign: NSString = NSString(string: "\(kSignStr1)10\(page)")
        
        sign = sign.md5String().md5String()
        let url = "http://api.wws.xywy.com/default.php?level=1&act=Hospital&fun=Hospital&version=2&os=&source=app_guahaozhushou&illid=&speciality_id=&province=&city=&PageNum=\(page)&pagesize=10&sign=\(sign)"
